"""
Example script for running OnMAR Design Prediction with CNN Configuration Application.

This demonstrates how to use OnMAR design prediction with the CNN configuration application
...
def run_onmar_cnn_example(
    ...
    """
    Run OnMAR design prediction with CNN configuration application.
    ...
    """
    print(f"\n{'='*60}")
    print(f"Running OnMAR Design Prediction with CNN Configuration")
    ...
    # Save model
    if save_results:
        model_path = f"OnMAR/design-prediction/models/cnn_{dataset_name}_{meta_learner_type}.pkl"
...
def plot_results(results: dict, dataset_name: str, meta_learner_type: str):
    """
    Plot OnMAR design prediction results.
    ...
    ax1.axvline(x=theta_t, color='r', linestyle='--', label=f'θt = {theta_t} (start predicting designs)')
    ax1.axhline(y=results['theta_p'], color='g', linestyle='--', label=f'θp = {results["theta_p"]} (pruning threshold)')
    ax1.set_xlabel('Timestep (Epoch)', fontsize=12)
    ax1.set_ylabel('Performance (Accuracy)', fontsize=12)
    ax1.set_title(f'OnMAR Design Prediction Performance Over Time\n{dataset_name.upper()} - {meta_learner_type.upper()}', fontsize=14)
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    # Plot 2: Design algorithm usage vs prediction
    phase1_calls = min(theta_t, len(timesteps))
    phase2_calls = results['num_design_algorithm_calls'] - phase1_calls
    phase2_predictions = results['num_design_predictions']

    categories = ['Phase 1\n(Before θt)', 'Phase 2\n(After θt)']
    algorithm_calls = [phase1_calls, phase2_calls]
    predictions = [0, phase2_predictions]

    x = range(len(categories))
    width = 0.35

    ax2.bar([i - width/2 for i in x], algorithm_calls, width, label='Design Algorithm Calls', color='orange')
    ax2.bar([i + width/2 for i in x], predictions, width, label='Design Predictions', color='blue')

    ax2.set_xlabel('Phase', fontsize=12)
    ax2.set_ylabel('Count', fontsize=12)
    ax2.set_title(f'Design Algorithm Usage vs. Meta-Learner Predictions\n{dataset_name.upper()} - {meta_learner_type.upper()}', fontsize=14)
    ax2.set_xticks(x)
    ...
    # Save plot
    plot_path = f"OnMAR/design-prediction/results/cnn_{dataset_name}_{meta_learner_type}.png"
...
def compare_meta_learners(dataset_name: str = 'mnist', timesteps: int = 50):
    ...
    print(f"{'='*60}")
    print(f"{'Meta-Learner':<15} {'Best Perf':<12} {'Final Perf':<12} {'Test Perf':<12} {'Predictions %':<15} {'Runtime (s)':<12}")
    print("-" * 90)
    ...
    for meta_learner, results in all_results.items():
        print(f"{meta_learner.upper():<15} "
              ...
              f"{results['design_prediction_percentage']:<15.1f} "
