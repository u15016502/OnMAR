"""
Example script for running OnMAR Design Prediction with Fuzzy ART Generation Application.

This demonstrates how to use OnMAR design prediction with the Fuzzy ART choice function generation application.
"""
...
def run_onmar_fuzzyart_example(
    dataset_name: str = 'enron',
    meta_learner_type: str = 'knn',
    timesteps: int = 40
):
    """
    Run OnMAR design prediction with Fuzzy ART generation application.

    Args:
        dataset_name: Dataset to use ('chatgpt', 'enron', 'imdb')
        meta_learner_type: Meta-learner to use ('knn', 'rf', 'xgboost')
        timesteps: Number of GE generations

    Returns:
        Results dictionary
    """
    print(f"\n{'='*60}")
    print(f"Running OnMAR Design Prediction with Fuzzy ART Generation")
    ...
    # Save model
    model_path = f"OnMAR/design-prediction/models/fuzzyart_{dataset_name}_{meta_learner_type}.pkl"
...
if __name__ == "__main__":
    # Example: Run with kNN (recommended for Fuzzy ART)
    results = run_onmar_fuzzyart_example(
        dataset_name='enron',
